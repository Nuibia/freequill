...
export const workflow = defineSequentialWorkflow({
  ...
  steps: [
    { id: 'selection', kind: 'subworkflow', workflow: 'selection@2', input: ({ input }) => ({ ...input.selection, genre: input.genre ?? 'dushi-naodong', mode: 'fast' }) },
    { id: 'scaffold', kind: 'subworkflow', workflow: 'scaffold@2', input: ({ input, results }) => ({ selection_artifact_ref: results.selection.artifact_ref, title: input.title ?? title(results), form: 'short', genre: input.genre ?? 'dushi-naodong' }) },
    { id: 'genre', kind: 'subworkflow', workflow: 'genre-diagnose@2', input: ({ input, results }) => ({ genre: input.genre ?? 'dushi-naodong', premise: resultOutput(results, 'selection').premise.selection.candidate, title: title(results) }) },
    {
      ...
      input: ({ input, results }) => ({ mode: 'fast', book_path: resultOutput(results, 'scaffold').book_path, genre: input.genre ?? 'dushi-naodong', topic_package: resultOutput(results, 'selection'), genre_diagnosis: resultOutput(results, 'genre'), brief: input.brief ?? null }),
    },
    {
      id: 'submission', kind: 'subworkflow', workflow: 'submission@2',
      input: ({ input, results }) => ({ book_path: resultOutput(results, 'scaffold').book_path, genre: input.genre ?? 'dushi-naodong', platform: input.platform ?? 'undecided', topic_package: resultOutput(results, 'selection'), delivery: resultOutput(results, 'create') }),
    },
  ],
