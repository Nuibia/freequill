...
export const workflow = defineSequentialWorkflow({
  ...
  steps: [
    ...ROLES.map(([role, capability]) => ({
      ...
      policyRefs: ['policies/review/short.v2.json'], isolation: { required: true, reason: '生产验稿必须与起草执行者隔离' },
      input: ({ input }) => ({ role, body: input.body, book_path: input.book_path ?? null, attempt: input.attempt ?? 1 }),
    })),
    {
