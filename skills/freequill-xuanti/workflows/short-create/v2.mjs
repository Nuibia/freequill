...
export const workflow = defineSequentialWorkflow({
  ...
  steps: [
    ...
    { id: 'outline-eval', kind: 'capability', capability: 'evaluate-short-outline@1', outputArtifactType: 'outline-evaluation', policyRefs: ['policies/creation/short.v2.json'], isolation: { required: true, reason: '细纲必须由隔离执行者评审' }, input: ({ results }) => ({ story_engine: resultOutput(results, 'story-engine').story_engine, outline: resultOutput(results, 'outline').outline }) },
    {
      ...
      input: ({ results }) => ({ story_engine: resultOutput(results, 'story-engine').story_engine, outline: resultOutput(results, 'outline').outline, evaluations: { story_engine: resultOutput(results, 'story-engine-eval'), outline: resultOutput(results, 'outline-eval') } }),
    },
    { id: 'story-engine-eval-2', kind: 'capability', capability: 'evaluate-story-engine@1', outputArtifactType: 'story-engine-evaluation', policyRefs: ['policies/creation/short.v2.json'], isolation: { required: true, reason: '返修故事引擎必须重新隔离评审' }, when: ({ results }) => Boolean(results['plan-revision']), input: ({ results }) => ({ story_engine: storyEngine(results), outline: outline(results) }) },
    { id: 'outline-eval-2', kind: 'capability', capability: 'evaluate-short-outline@1', outputArtifactType: 'outline-evaluation', policyRefs: ['policies/creation/short.v2.json'], isolation: { required: true, reason: '返修细纲必须重新隔离评审' }, when: ({ results }) => Boolean(results['plan-revision']), input: ({ results }) => ({ story_engine: storyEngine(results), outline: outline(results) }) },
    {
    ...
    { id: 'review-1', kind: 'subworkflow', workflow: 'review-short@2', input: ({ input, results }) => ({ book_path: input.book_path, body: resultOutput(results, 'draft'), attempt: 1 }) },
    { id: 'revision-1', kind: 'capability', capability: 'revise-short-story@1', outputArtifactType: 'short-revision-package', policyRefs: ['policies/creation/short.v2.json'], when: ({ results }) => reviewVerdict(results, 'review-1') === 'FIX_BODY', input: ({ results }) => ({ draft: resultOutput(results, 'draft'), review: resultOutput(results, 'review-1'), attempt: 1 }) },
    { id: 'review-2', kind: 'subworkflow', workflow: 'review-short@2', when: ({ results }) => Boolean(results['revision-1']), input: ({ input, results }) => ({ book_path: input.book_path, body: resultOutput(results, 'revision-1'), attempt: 2 }) },
    ...
    { id: 'review-3', kind: 'subworkflow', workflow: 'review-short@2', when: ({ results }) => Boolean(results['revision-2']), input: ({ input, results }) => ({ book_path: input.book_path, body: resultOutput(results, 'revision-2'), attempt: 3 }) },
    {
